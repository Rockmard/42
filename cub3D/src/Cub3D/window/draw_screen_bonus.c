...
/*   Updated: 2024/12/18 15:37:30 by nrontey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3D.h"

static void	draw_tile_border(t_data *data, int x, int y, int size_modifier)
{
	int	i;
	int	j;
	int	border_size;
	...
	while (i < size_modifier)
	{
		...
		{
			if (is_border_pixel(i, j, size_modifier, border_size))
				draw_pixel_if_valid(data, x * size_modifier + i, \
...
int	update_minimap(t_data *data, int size_modifier)
{
	...
	max_size = fmin(WIDTH / data->map->map_width, \
	HEIGHT / data->map->map_height);
	adjusted_size = fmin(size_modifier, max_size);
	render_3d_view(data);
	if (data->map->minimap_display)
	{
		put_minimap(data, adjusted_size);
	}
